...
public class MezzBean implements TreeNodeResource {
    ...
    public void setMezzMac(String mezzMac) {
        ...
        if (this.mezzETHMac == null || this.mezzETHMac.length() == 0 || !this.mezzMac.startsWith("0;")) {
            return;
        }

        String[] macAry = this.mezzMac.replaceAll("^0;", "").split(";");

        List<String> mezzETHMacList = new ArrayList<>();
        List<String> mezzIBMacList = new ArrayList<>();
        List<String> mezzFCWWPNList = new ArrayList<>();

        for (String mac : macAry) {
            if (!mac.matches("^(0|1|2)#.+")) {
                continue;
            }
    ...
    public ResourceKey convert2Resource(String id, String adapterKind,
        ...
        metricData.add(new MetricData(new MetricKey(false).add(Constant.ATTR_MEZZ_HEALTH_STATUS), timestamp,
            ConvertUtils.convertHealthState(this.getMezzHealthStatus())));
        metricData.add(new MetricData(new MetricKey(true).add(Constant.ATTR_PRESENTSTATE), timestamp,
        ...
        metricData.add(
            new MetricData(new MetricKey(true).add(Constant.ATTR_MEZZ_LOCATION), timestamp, this.mezzLocation));
