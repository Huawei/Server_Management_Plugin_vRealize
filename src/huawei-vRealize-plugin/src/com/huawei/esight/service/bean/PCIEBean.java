...
public class PCIEBean implements TreeNodeResource {
    ...
    public ResourceKey convert2Resource(String dn, String adapterKind,
        ...
        metricDataList.add(new MetricData(new MetricKey(false, Constant.ATTR_PCIESSDCARD_HEALTHSTATUS), timestamp,
            ConvertUtils.convertHealthState(this.pcieSsdCardHealthStatus)));
        metricDataList.add(new MetricData(new MetricKey(true, Constant.ATTR_PCIESSDCARD_LIFELEFT), timestamp,
            this.pcieSsdCardLifeLeft));
        metricDataList.add(new MetricData(new MetricKey(true, Constant.ATTR_UUID), timestamp, this.uuid));
        metricDataList.add(new MetricData(new MetricKey(true, Constant.ATTR_PCIESSDCARD_MANYFACTURER), timestamp,
            this.pciecardManufacturer));
